...
export function QuestionSlider({ question, initial, onSubmit }: Props) {
  ...
  return (
    <div className="flex flex-1 flex-col justify-between px-6 pb-12 pt-[15vh]">
      <div>
        <p className="font-serif text-[clamp(1.5rem,5.5vw,2.1rem)] leading-[1.18] tracking-tightest text-cream">
          {question.prompt}
        </p>
      </div>
      <div className="flex flex-col gap-10 pb-[env(safe-area-inset-bottom)]">
        <motion.p
          key={sliderLabel(question, value)}
          initial={{ opacity: 0, y: 4 }}
          animate={{ opacity: touched ? 1 : 0.5, y: 0 }}
